...
function laborator_init() {
	...
	// Admin JS & CSS
	wp_register_script( 'admin-js', kalium()->assetsUrl( 'js/admin/main.min.js' ), array( 'jquery' ), $version );
 ...
 */
function kalium_wp_enqueue_scripts() {
	...
	// Single Post
	if ( is_single() ) {
		wp_enqueue_script( 'jquery-throttle-debounce', kalium()->assetsUrl( 'js/jquery.ba-throttle-debounce.js' ), null, null, true );
...
add_action( 'wp_enqueue_scripts', 'kalium_wp_enqueue_scripts', 10 );
 ...
 */
function kalium_wp_footer() {

	// Kalium main JS file main.min.js
	wp_enqueue_script( 'kalium-main' );
}

add_action( 'wp_footer', 'kalium_wp_footer' );
	...
	if ( ! has_site_icon() && ( $favicon_image || $apple_touch_icon ) ) {
		...
		<?php if ( $apple_touch_icon ) : ?>
            ...
            <link rel="apple-touch-icon-precomposed" sizes="114x114"
                  href="<?php echo esc_attr( $apple_touch_icon ); ?>">
		<?php endif; ?>
		<?php
	}
}
...
function kalium_google_theme_color() {
	if ( $google_theme_color = get_data( 'google_theme_color' ) ) {
		?>
        <meta name="theme-color" content="<?php echo esc_attr( $google_theme_color ); ?>"><?php
	}
}
...
function kalium_check_for_custom_logo_in_page() {
	...
	if ( $custom_menu_skin && in_array( $custom_menu_skin, array(
			'menu-skin-main',
			'menu-skin-dark',
			'menu-skin-light'
		) ) ) {
...
function kalium_go_to_top_link() {
	...
	?>
    <a href="#top" class="go-to-top<?php echo $button_type == 'circle' ? ' rounded' : '';
	echo ' position-' . $position; ?>" data-type="<?php echo $type; ?>" data-val="<?php echo in_array( $type, array(
		'pixels',
		'percentage'
	) ) ? intval( $activate_when ) : esc_attr( $activate_when ); ?>">
...
/**
 * Upgrading to Kalium v2.5.2
 */
function kalium_version_upgrade_to_2_5_2() {
	// Previous update v2.5.1 has broken Custom CSS quotes, so now when the theme is updated automatic fix will run
	laborator_set_custom_css( get_option( 'laborator_custom_css' ) );
	laborator_set_custom_css( get_option( 'laborator_custom_css_lg' ), 'lg' );
	laborator_set_custom_css( get_option( 'laborator_custom_css_md' ), 'md' );
	laborator_set_custom_css( get_option( 'laborator_custom_css_sm' ), 'sm' );
	laborator_set_custom_css( get_option( 'laborator_custom_css_xs' ), 'xs' );
}

add_action( 'kalium_version_upgrade_2_5_2', 'kalium_version_upgrade_to_2_5_2', 10 );

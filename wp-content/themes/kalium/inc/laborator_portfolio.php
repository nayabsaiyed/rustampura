...
function laborator_acf_location_rules_match_field_groups( $field_groups = array(), $options = array() ) {
	...
	if ( isset( $options['item_type'] ) ) {
		...
		$current_item_type = kalium()->acf->get_field( 'item_type', $post_id );
...
function kalium_portfolio_item_type_column_content_action( $column, $post_id ) {
	...
	if ( $column == 'item_type' ) {
		...
		$item_type = kalium()->acf->get_field( 'item_type' );
		...
		if ( isset( $item_types[ $item_type ] ) ) :
			...
			?>" class="portfolio-item-type-column">
                <img src="<?php echo kalium()->assetsUrl( 'images/admin/portfolio-item-' . $item_type . '.png' ); ?>"/>
...
function kalium_portfolio_item_type_filter_dropdown() {
	...
	if ( $pagenow == 'edit.php' && $typenow == 'portfolio' ) {
		...
		$current_item_type = kalium()->url->get( 'portfolio_item_type' );
...
function kalium_portfolio_item_type_filter_query_request( $query ) {
	...
	$item_type = kalium()->url->get( 'portfolio_item_type' );
...
function kalium_portfolio_get_lightbox_settings_and_items( $items, $gallery_id = 'portfolio-slider' ) {
	...
	// Lightbox Skin
	$lightbox_skin = get_data( 'portfolio_lb_skin' );
	...
	// Share 
	if ( get_data( 'portfolio_share_item' ) ) {
...
function kalium_portfolio_item_lightbox_entry( $wp_post ) {
	...
	switch ( $content_to_show ) {
		...
		case 'gallery':
			$gallery = kalium()->acf->get_field( 'image_and_video_gallery', $post_id );
		...
		case 'self-hosted-video':
			$self_hosted_video = kalium()->acf->get_field( 'self_hosted_video', $post_id );
...
function kalium_get_portfolio_query( $opts = array() ) {
	...
	$args = array(
		...
		'layouts'                   => array(
			...
			'type_1' => array(
				...
				'default_spacing'        => get_data( 'portfolio_type_1_default_spacing' ),
	...
	if ( ! empty( $post_id ) && $vc_mode == false && ( $portfolio = get_post( $post_id ) ) ) {
		...
		if ( $portfolio->masonry_style_portfolio ) {
			$args['custom_query']     = array();
			$args['layout_type']      = 'type-2';
			$args['masonry_items']    = kalium()->acf->get_field( 'masonry_items_list', $portfolio );
	...
	if ( empty( $opts['no_query'] ) ) {
		...
		if ( ! defined( 'DOING_AJAX' ) ) {
			...
			if ( isset( $args['masonry_items_ids'] ) ) {
				...
				generate_custom_style( $portfolio_instance_id_attr . '.portfolio-holder.merged-item-spacing .thumb .hover-state.hover-distanced', "left: {$thumbnail_spacing}px; right: {$thumbnail_spacing}px; top: {$thumbnail_spacing}px; bottom: {$thumbnail_spacing}px;" );

				// Spacing for distanced hover background
				$spacing += 15;
				generate_custom_style( $portfolio_instance_id_attr . '.portfolio-holder .thumb .hover-state.hover-distanced', "left: {$thumbnail_spacing}px; right: {$thumbnail_spacing}px; top: {$thumbnail_spacing}px; bottom: {$thumbnail_spacing}px;" );
			}
			...
			if ( 'type-1' == $args['layout_type'] && is_numeric( $args['layouts']['type_1']['default_spacing'] ) ) {
				...
				generate_custom_style( '.portfolio-holder', "margin-left: {$container_spacing}px; margin-right: {$container_spacing}px;" );
				generate_custom_style( '.portfolio-holder .portfolio-item.has-padding', "padding: {$spacing}px;" );
			}
		}
	}
...
function kalium_portfolio_generate_portfolio_instance_object( $portfolio_args ) {
	...
	if ( function_exists( 'icl_object_id' ) && ! empty( $base_query['post__in'] ) ) {
...
function portfolio_items_get_from_ajax() {
	...
	$vc_attributes = kalium()->post( 'vcAttributes' );
	$post_id       = kalium()->post( 'postId' );
	$shown_ids     = kalium()->post( 'shownIds' );
	$count         = kalium()->post( 'count' );
	$count_terms   = kalium()->post( 'countByTerms' );

	// Show all items, ignore category var
	$no_category = kalium()->post( 'noCategory' );

	// Get Single Category of Items
	$get_category = kalium()->post( 'portfolioCategory' );
		...
		foreach ( $sub_terms as $term ) :
			...
			when_match( $is_active, 'active' ); ?>">
                <a href="<?php echo esc_url( $term_link ); ?>"
                   data-term="<?php echo esc_attr( $term->slug ); ?>"><?php echo esc_html( $term->name ); ?></a>
            </li>
		<?php
		endforeach;
		?>
...
function kalium_portfolio_generate_gallery_gap( $gap = 30, $type = '' ) {
	...
	if ( $type == 'carousel' ) {
		generate_custom_style( '.single-portfolio-holder.portfolio-type-3 .gallery-slider .gallery-item', "margin-left:{$gap_half}px; margin-right:{$gap_half}px;" );
	} else {
		generate_custom_style( '.single-portfolio-holder .gallery .photo, .single-portfolio-holder .gallery .portfolio-video, .single-portfolio-holder .gallery .portfolio-images-slider, .single-portfolio-holder .gallery .comparison-image-slider', "margin-bottom:{$gap}px;" ); //@ToDo
...
function laborator_remove_portfolio_prefix_from_link( $post_link, $post, $leavename ) {
	...
	$custom_prefix = get_data( 'portfolio_prefix_url_slug' );
...
if ( in_array( get_data( 'portfolio_url_add_prefix' ), array( '', 0 ) ) ) {

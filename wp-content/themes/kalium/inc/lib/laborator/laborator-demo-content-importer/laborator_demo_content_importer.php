...
function lab_1cl_demo_installer_get_packs() {
	return array(
		...
		array(
			...
			'options-extend' => 'demo-content/automotive/options-extend.json',
			'layerslider'    => '',
			'revslider'      => array(
		...
		array(
			...
			'revslider'   => array(
				...
				'demo-content/architecture/revslider-home.zip',
			),
		...
		if ( isset( $pack['requires'] ) && is_array( $pack['requires'] ) && count( $pack['requires'] ) ) {
			...
			if ( in_array( 'prdctfltr', $pack['requires'] ) ) {
				...
				if ( ! in_array( $prdctfltr_plugin, $active_plugins ) && ! is_plugin_active_for_network( $prdctfltr_plugin ) ) {
					$file_content_pack['checked']               = false;
					$file_content_pack['disabled']              = true;
					$file_content_pack['requires']['prdctfltr'] = 'This content pack required <strong>WooCommerce Product Filter</strong> plugin to be installed and activated. To install it go to <a href="' . esc_url( admin_url( "themes.php?page=kalium-install-plugins" ) ) . '" target="_blank">Appearance &raquo; Install Plugins</a> and then refresh this page.';
				}
			}
...
function lab_1cl_demo_install_package_content() {
	...
	switch ( $source_details['type'] ) {
		...
		case "theme-options":
			...
			if ( isset( $pack['options-extend'] ) && $pack['options-extend'] ) {
				$general_options = lab_1cl_demo_installer_get_file_from_path( $pack['options-extend'] );

				try {
					if ( $_general_options = file_get_contents( $general_options ) ) {
						$_general_options = maybe_unserialize( base64_decode( $_general_options ) );

						foreach ( $_general_options as $option_id => $option_value ) {

...
function kalium_acfpro_gm_init() {
	...
	wp_register_style( 'kalium-acfpro-gm-main-css', kalium()->locateFileUrl( 'inc/lib/laborator/laborator-acfpro-grouped-metaboxes/laborator-acfpro-grouped-metaboxes.min.css' ), null, kalium()->getVersion() );
	...
	if ( in_array( $pagenow, array( 'post.php', 'post-new.php' ) ) ) {
		...
		foreach ( acf()->local->groups as $group_id => $group ) {
			if ( in_array( $group['key'], $allowed_field_groups ) ) {
				$kalium_acfpro_gm_groups[ $group_id ] = array(
					'title' => $group['title'],

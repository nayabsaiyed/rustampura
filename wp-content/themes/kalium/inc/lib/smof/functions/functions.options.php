...
$of_options[] = array( 	'desc' 		=> "Set maximum width for the uploaded logo, mostly used when you use retina (@2x) logo<br /><small>{$note_str} Even if you don't upload custom logo, you may set custom width for the current logo in sticky menu.</small>",
	...
	*/
		...
		$of_options[] = array(
			'name' => 'Custom JavaScript Information',
			'desc' => "",
			'id'   => 'portfolio_lb',
			'std'  => "
...
// BLOG SETTINGS
		...
		$of_options[] = array(
			'desc'    => "Featured image thumbnail height (applied on single post only). If you change this value, you need to <a href=\"admin.php?page=laborator-docs#regenerate-thumbnails\" target=\"_blank\">regenerate thumbnails</a> again",
...
// PORTFOLIO SETTINGS
		...
		$of_options[] = array(
			'desc'  => "Thumbnail size for this type<br><small>If you change dimensions you must <a href=\"admin.php?page=laborator-docs#regenerate-thumbnails\" target=\"_blank\">regenerate thumbnails</a>. <br>{$thumbnail_sizes_info}</small>",
...
// Portfolio Loop
		...
		if ( $portfolio_prefix_url_slug_placeholder ) {
			...
			$of_options[] = array(
				'desc' => "Custom portfolio category URL prefix<br><small>{$note_str} When you change this setting you need to <a href=\"" . admin_url( 'admin.php?page=laborator-docs#flush-rewrite-rules' ) . "\" target=\"_blank\">flush rewrite rules</a>.</small>",
...
// SHOP SETTINGS
		...
		if ( false == is_shop_supported() ) {
			...
			function kalium_woocommerce_product_categories_name_replace( $item ) {
				return str_replace( array( 'product per row', 'products per row' ), array(
					'category per row',
					'categories per row'
				), $item );
			}
			...
			$of_options[] = array(
				'desc' => "Category Image Size <br /><small>If you change dimensions you must <a href=\"admin.php?page=laborator-docs#regenerate-thumbnails\" target=\"_blank\">regenerate thumbnails</a>.<br>{$thumbnail_sizes_info}</small>",
...
// ! Shop: Single
			...
			$of_options[] = array(
				...
				'desc'  => "Custom image size<br><small class=\"note\">{$note_str} Enabling this option will affect only <strong>main product images</strong>. <a href=\"" . admin_url( 'admin.php?page=laborator-docs#regenerate-thumbnails' ) . "\" target='_blank'>Thumbnails regeneration</a> is required.</small>",
...
// OTHER SETTINGS
		...
		$post_types_obj = get_post_types( array(
			'_builtin'            => false,
...
// END OF OTHER SETTINGS
		...
		if ( 'admin.php' == $pagenow && 'laborator_options' === kalium()->url->get( 'page' ) ) {
			$custom_skin_path = kalium_get_custom_skin_file_path( true );
			$custom_skin_path_relative = kalium_get_custom_skin_file_path();
...
// END OF BORDERS
		...
		$of_options[] = array(
			'desc' => 'Icon (optional)<br><small>Note: If you want to set custom icon, enter icon alias from <a href="https://fontawesome.com/v4.7.0/icons/" target="_blank">Font Awesome</a> icon collection.</small>',
